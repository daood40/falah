...
class HadithRepository {
  ...
  /// What independent corpora say about one record — verdicts only, no text.
  Future<List<Map<String, dynamic>>> crossChecks(String hadithId) async {
    final res = await _api.get('/api/v1/hadiths/$hadithId/cross-checks');
    return (res.data as List).cast<Map<String, dynamic>>();
  }

  Future<List<Map<String, dynamic>>> collections() async {
    final res = await _api.get('/api/v1/collections', query: {'limit': 100});
    return (res.data as List).cast<Map<String, dynamic>>();
  }

  Future<List<Map<String, dynamic>>> gradings() async {
  ...
  Future<List<Map<String, dynamic>>> catalog({bool chapters = false}) async {
    final res = await _api.get('/api/v1/catalog', query: chapters ? {'chapters': 'true'} : null);
    return (res.data as List).cast<Map<String, dynamic>>();
  }
