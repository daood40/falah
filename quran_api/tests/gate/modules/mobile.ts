...
export async function run(ctx: GateContext): Promise<void> {
  ...
  if (aapt) {
    ...
    const badgingChecks: { id: string; description: string; ok: boolean; expected: string; actual: string; severity: 'CRITICAL' | 'HIGH' | 'MEDIUM' | 'LOW' }[] = [
      ...
      { id: 'PERMISSIONS', description: 'the built APK requests only the INTERNET permission', ok: (badging.match(/uses-permission: name='([^']+)'/g) ?? []).every((line) => line.includes('android.permission.INTERNET') || line.includes('DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION')), expected: 'INTERNET only (plus the self-scoped receiver permission AndroidX adds on API 33+)', actual: (badging.match(/uses-permission: name='([^']+)'/g) ?? []).join(', ') || 'none', severity: 'CRITICAL' },
      ...
      { id: 'MIN-SDK', description: 'the built APK declares a minimum API level', ok: Number.parseInt((badging.match(/minSdkVersion:'(\d+)'/) ?? badging.match(/(?<![a-zA-Z])sdkVersion:'(\d+)'/) ?? [])[1] ?? '0', 10) >= 21, expected: 'minSdkVersion >= 21', actual: (badging.match(/minSdkVersion:'(\d+)'/) ?? badging.match(/(?<![a-zA-Z])sdkVersion:'(\d+)'/) ?? [])[1] ?? 'unknown', severity: 'MEDIUM' },
