...
async function main(): Promise<void> {
  ...
  writeMasterReport(gate, { reports, startedAt, finishedAt, timings, partial: only.length > 0 });
  ...
  const evidence = path.join(reports, 'gate-evidence.jsonl');
  ...
  const shortfalls = only.length > 0
    ? []
  ...
  if (totals.fail > 0 || shortfalls.length > 0 || (only.length === 0 && totals.total < 20_000)) {
    process.exitCode = 1;
  }
}
