-- 008 — Rukus (الركوعات): the 556 Hafs rukuʿ boundaries.
--
-- Every ayah already carries ruku_number (from quran-meta, MIT); this table
-- gives the boundaries their own rows, exactly like juzs/pages/manzils, so the
-- API can list and address a ruku directly. The boundaries come only from the
-- source dataset — nothing here is composed.

create table quran.rukus (
  id uuid primary key default gen_random_uuid(),
  edition_id uuid not null references quran.quran_editions (id) on delete cascade,
  ruku_number integer not null check (ruku_number between 1 and 556),
  ...
  unique (edition_id, ruku_number)
);

create index rukus_surah_idx on quran.rukus (edition_id, surah_number, ruku_number);
create index ayahs_ruku_idx on quran.ayahs (edition_id, ruku_number, global_ayah_number);

-- Same posture as every other structural table: read for anon/authenticated,
-- writes only for service_role, RLS forced.
alter table quran.rukus enable row level security;
alter table quran.rukus force row level security;
create policy rukus_public_read on quran.rukus for select to anon, authenticated using (true);
grant select on quran.rukus to anon, authenticated;
grant select, insert, update, delete on quran.rukus to service_role;

-- The catalogue view enumerates every category in one union, so it is
-- replaced with the same body as 006 plus the rukus row.
...
with base as (
  ...
  union all select 'manzils', 'المنازل', 'manzils', 'metadata',
         ...
         'none', 0, null
  union all select 'rukus', 'الركوعات', 'rukus', 'metadata',
         (select count(*) from quran.rukus)::int,
         (select count(*) from quran.rukus where verified)::int,
         (select string_agg(distinct source_id, ', ') from quran.rukus), null,
         'none', 0, '556 rukuʿ boundaries; every ayah also carries ruku_number'
