...
## أين توقّفنا
2026-09-17 (تصنيفات القرآن كاملة): أُضيف جدول `quran.rukus` (هجرة 008، 556 ركوعًا
من quran-meta) ومسارات `/rukus[?surah=]`, `/rukus/{n}[/ayahs]`, `/rubs`, `/rubs/{n}`,
`/pages`, `/manzils/{n}`، وتصفية السور `?revelation=makkah|madinah&sort=revelation_order`،
و`structure` في `/surahs/{id}` (صفحات/أجزاء/أحزاب/منزل/عدد الركوعات والسجدات).
للتطبيق: ملف بنية بلا نصّ `flutter_app/assets/quran/structure.json` يولَّد من المصدر
(`npm run quran:structure`، تفحصه البوابة بايتًا بايت) وشاشة «تصفح المصحف» بسبع
تبويبات + تصفية مكي/مدني وترتيب النزول في قائمة السور. OpenAPI 60 مسارًا. اختبارات
الـAPI 106/106، وبوابة CI كاملة (تشغيل 22): 41,599 حالة بلا FAIL/BLOCKED/SKIPPED.
...
2026-09-17 (بوابة 20,000+ اختبار): `npm run gate:tests` في quran_api نفّذت
39,413 حالة حقيقية على PostgreSQL وخادم HTTP حقيقيين: 38,747 PASS · 0 FAIL ·
0 SKIPPED · 666 BLOCKED (أندرويد/iOS/Docker بلا أدوات هنا). تغطية كاملة
114/114 سورة و6,236/6,236 آية عبر كل الطبقات، 6,236 محاولة تعديل مرفوضة
بـSOURCE_LOCK، 228 محاولة تلاعب كُشفت ورُوجعت، ونسخ/استرجاع حقيقي مرتين.
البوابة كشفت ستة عيوب حقيقية وأُصلحت كلها: بحث exact=true كان يعيد 500،
ترميز نسبة فاسد في المسار 500، بايت NUL 500، جسم طلب ضخم يقطع الاتصال،
جدولا import_runs/audit_logs بلا سياسة RLS (هجرة 007)، وحالة جلسة متسرّبة
على اتصال مجمّع. لكل عيب حارس انحدار. القرار: BLOCKED (تراخيص + اعتماد بشري).
