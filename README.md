...
لا تعتمد على بقية هذا المستودع — انسخ مجلد `hadith_api/` وحده واستعملها في أي مشروع.
...
**15,959 حديثًا** (إصدار `JAMI-KAMIL-1437-V2`) · 67 كتابًا · 5,338 بابًا · 1,643 راويًا · 26,934 رابط تخريج.
...
1. **مطابقة المصدر** (`npm run sample-verify`): كل سجل من الـ15,959 يُعاد قياسه
   مقابل ملف المصدر: النص حرفيًا، والصفحة المُعلنة، وبصمة SHA-256 — 15959/15959.
...
## تدقيق ما قبل الإطلاق

```bash
npm run staging:up        # قاعدة مستعادة من نسخة احتياطية + خادم على منفذ مستقل
npm run audit             # 274,881 فحصًا مستقلًا عبر سبع عائلات
npm run staging:down
```

كل سطر في `reports/FINAL_QA_AUDIT.txt` فحص واحد على كائن مختلف (سجل، منفذ، دور،
حمولة، ملف). عائلات HTTP تنادي البيئة المنشورة من **خارج** الخادم، ولا تستدعي
